...
def output_lc_info(output_media=None, lc=None):
    ...
    if output_media == 'console':
        ...
        td.append(['created', str(dash_if_none(lc.get('CreatedTime').replace(tzinfo=None, microsecond=0)))])
        td.append(['iam instance profile', dash_if_none(lc.get('IamInstanceProfile'))])
        td.append(['ebs optimised', dash_if_none("Yes" if lc.get('EbsOptimized') else "No")])
        td.append(['instance monitoring', dash_if_none("Yes" if lc.get('InstanceMonitoring').get('Enabled') else "No" )])
        td.append(['classiclink vpc security groups', dash_if_none(lc.get('ClassicLinkVPCSecurityGroups'))])
        td.append(['block device mappings', dash_if_none(lc.get('BlockDeviceMappings'))])
        td.append(['key name', dash_if_none(lc.get('KeyName'))])
        td.append(['security groups', dash_if_none(", ".join(lc.get('SecurityGroups')))])
        td.append(['kernel ID', dash_if_none(lc.get('KernelId'))])
        td.append(['ramdisk ID', dash_if_none(lc.get('RamdiskId'))])
        td.append(['image ID', dash_if_none(lc.get('ImageId'))])
        td.append(['Instance Type', dash_if_none(lc.get('InstanceType'))])
        td.append(['associate public IP', dash_if_none(lc.get('AssociatePublicIpAddress'))])
        td.append(['user data', dash_if_none(lc.get('UserData'))])
